resource "azurerm_sql_firewall_rule" "example" {
  name                = "pingsafe-vuln-firewall-rule-${var.environment}"
...
resource "azurerm_sql_server" "example" {
  name                         = "pingsafe-vuln-sqlserver-${var.environment}${random_integer.rnd_int.result}"
  resource_group_name          = azurerm_resource_group.example.name
...
resource "azurerm_mysql_server" "example" {
  name                = "pingsafe-vuln-mysql-${var.environment}${random_integer.rnd_int.result}"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name

  administrator_login          = "pingsafe-vuln-${var.environment}"
...
resource "azurerm_postgresql_server" "example" {
  name                         = "pingsafe-vuln-postgresql-${var.environment}${random_integer.rnd_int.result}"
  location                     = azurerm_resource_group.example.location
  resource_group_name          = azurerm_resource_group.example.name
  sku_name                     = "B_Gen5_2"
  storage_mb                   = 5120
  ...
  auto_grow_enabled            = true
  administrator_login          = "pingsafe-vuln"
  administrator_login_password = "Aa12345678"
  version                      = "9.5"
  ssl_enforcement_enabled      = false
  tags = {
